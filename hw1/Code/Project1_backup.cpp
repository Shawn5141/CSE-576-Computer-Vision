...
{
    // Add your code here
    int size = kernelWidth*kernelHeight; // This is the size of the kernel
    int buffer_imageWidth=imageWidth+kernelWidth;
    int buffer_imageHeight=imageHeight+kernelHeight;


    //Create a buffer with kernel size added
    double **buffer = new double *[buffer_imageHeight] ;
    for (int M=0;M<buffer_imageHeight;M++){
        buffer[M]=new double [buffer_imageWidth];
    }


    //Padding the zero on the buffer with image value
    for (int m=0;m<buffer_imageHeight;m++){
        for (int n=0;n<buffer_imageWidth;n++){
           if (m< int(imageHeight/2) || m>=imageHeight+int(kernelHeight/2) || (n<int(imageWidth/2) || n>=imageWidth+int(kernelWidth/2))){

                   buffer[m][n]=0.0;
                   //std::cout<<" what :"<<buffer[m][n]<<std::endl;
            }
           else{
               buffer[m][n]=Image[m-int(kernelHeight/2)][n-int(kernelWidth/2)];
               //std::cout<<" Image :"<<buffer[m][n]<<std::endl;
           }
           //std::cout<<" buffer :"<<buffer[m][n]<<std::endl;
        }
    }
    std::cout<<" buffersfjsd :"<<buffer[0][0]<<std::endl;
        ...
        for (int c = 0; c < imageWidth; c++){
            ...
            for(int rd=-int(kernelHeight/2);rd<=int(kernelHeight/2);rd++){
                for(int cd=-int(kernelWidth/2);cd<=int(kernelWidth/2);cd++){
                    ...
                    // Get the value of the kernel
                    double weight = kernel[kernelWidth*(rd+int(kernelHeight/2))+(cd+int(kernelWidth/2))];
                    //std::cout<<"r "<<r+int(kernelHeight/2)+rd<<std::endl;
                    //std::cout<<"c "<<c+int(kernelWidth/2)+cd<<"\n"<<std::endl;
                    value+=buffer[r+int(kernelHeight/2)+rd][c+int(kernelWidth/2)+cd]*weight;

                    //std::cout<<"x "<<buffer[r+int(kernelHeight/2)+rd][c+int(kernelWidth/2)+cd]*weight<<std::endl;
                }
            }
